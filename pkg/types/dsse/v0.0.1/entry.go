...
func parseIntotoStatement(p []byte) (*in_toto.Statement, error) {
	...
	payload, err := base64.StdEncoding.DecodeString(string(p))
	if err != nil {
		return nil, err
	}
...
func (v V001Entry) CreateFromArtifactProperties(_ context.Context, props types.ArtifactProperties) (models.ProposedEntry, error) {
	...
	paeEncodedPayload := dsse.PAE(env.PayloadType, []byte(env.Payload))
	h := sha256.Sum256(paeEncodedPayload)
	payload := strfmt.Base64([]byte(env.Payload))
	re.DsseObj.Payload = &payload
	...
	for _, sig := range env.Signatures {
		...
		if !ok {
			return nil, errors.New("all envelope signatures must be validated by a public key")
		}

		// similarly, the string is already base64 encoded
		...
		re.DsseObj.Signatures = append(re.DsseObj.Signatures, &models.DsseV001SchemaSignaturesItems0{
			Keyid:     sig.KeyID,
			Sig:       sigBytes,
			PublicKey: strfmt.Base64(key.keyBytes),
		})
	}
...
func verifyEnvelope(allPubKeyBytes [][]byte, env *dsse.Envelope) (map[string]verifier, error) {
	// generate a fake id for these keys so we can get back to the key bytes and match them to their corresponding signature
	verifiers := make([]dsse.Verifier, 0)
	verifiersById := make(map[string]*verifier)

	for i, pubKeyBytes := range allPubKeyBytes {
		...
		verifiers = append(verifiers, dsseVfr)
		verifiersById[dsseVfr.id] = dsseVfr
	}

	envVerifier, err := dsse.NewMultiEnvelopeVerifier(len(env.Signatures), verifiers...)
	if err != nil {
		return nil, fmt.Errorf("could not create dsse verifier: %w", err)
	}

	acceptedKeys, err := envVerifier.Verify(env)
	if err != nil {
		return nil, fmt.Errorf("could not verify envelope: %w", err)
	}

	verifierBySig := make(map[string]verifier)
	for _, acceptedKey := range acceptedKeys {
		verifier, ok := verifiersById[acceptedKey.KeyID]
		if !ok {
			continue
		}

		verifierBySig[acceptedKey.Sig.Sig] = *verifier
	}
